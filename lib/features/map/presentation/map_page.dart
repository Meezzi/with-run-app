...
class _MapPageState extends ConsumerState<MapPage> with WidgetsBindingObserver {
  ...
  void _tryMoveToCurrentLocation() {
    ...
    debugPrint(
      '위치 이동 시도 - 위치 정보: ${locationState.currentPosition}, 맵 컨트롤러: ${mapState.mapController.isCompleted}',
    );
  ...
  Future<void> _checkAndRestoreState() async {
    try {
      ...
      // 저장된 상태가 있는지 확인
      final hasState =
          prefs.containsKey('last_latitude') &&
  ...
  Future<void> _saveAppState() async {
    try {
      ...
      if (mapState.selectedPosition != null) {
        await prefs.setDouble(
          'selected_latitude',
          mapState.selectedPosition!.latitude,
        );
        await prefs.setDouble(
          'selected_longitude',
          mapState.selectedPosition!.longitude,
        );
  ...
  Future<void> _restoreAppState() async {
    try {
      ...
      // 마지막 위치로 카메라 이동
      if (prefs.containsKey('last_latitude') &&
          prefs.containsKey('last_longitude')) {
  ...
  void _setupMapCreatedCallback(LatLng target) {
    ref.listen(mapProvider.select((state) => state.mapController), (
      previous,
      current,
    ) {
