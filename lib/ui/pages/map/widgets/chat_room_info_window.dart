...
class _ChatRoomInfoWindowState extends ConsumerState<ChatRoomInfoWindow> {
  ...
  Future<void> _deleteChatRoom() async {
    ...
    try {
      ...
      // 채팅방 삭제 - 이제 채팅방 뷰모델의 leaveRoom 함수를 호출합니다.
      final chatRoomVm = ref.read(chatRoomViewModel.notifier);
      ...
      // 맵 새로고침 - 이제 맵 프로바이더의 refreshMap 함수를 명시적으로 호출합니다.
  ...
  void _showDeleteConfirmDialog() {
    // 다이얼로그 표시 전 인포 윈도우를 닫아 UI가 겹치지 않게 하고, 다이얼로그를 표시합니다.
    widget.onDismiss();
    
    // 다이얼로그 표시 - Future.microtask를 사용하여 함수 호출 스택 이후에 표시되도록 합니다.
    Future.microtask(() {
      if (mounted) {
        showDialog(
          context: context,
